...
- Browser tests pass for the noise-free 16-row queue and hover-only demo guidance, Lina's first-person statement, responsive dormant/active pane layouts, active lifecycle checks, compact stage-aware agent chat, transparent AI-authored preparation copy, automatic synthetic first-email playback without evaluator input, automatic and manually controlled in-place message expansion, slower writing and incoming-email states, customer-source registration of the updated estimate, automatic first exchange, manual updated-estimate continuation, newest-first communication ordering, handler-ready transition, the separate Erik/Samsung evidence, exact prompt-injection location marker, fail-closed specialist referral, audit log, grouped source viewer, and reset.
