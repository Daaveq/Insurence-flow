...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. The compact draft remains editable and requires explicit handler approval; after that synthetic send, customer typing, the missing-photo reply, AI processing, and the AI acknowledgement play automatically. The flow then pauses for exploration until the tester triggers the updated-estimate email, after which the agent prepares the handler handoff. Newest messages appear first in a conversational layout with customer bubbles on the left and AI bubbles on the right. Clicking a message expands that same bubble in place; newly arrived messages auto-expand for 3 seconds, customer writing remains visible for 5 seconds, received mail rests for 3 seconds, and subsequent AI processing takes about 4 seconds. No real email is sent.
- The lower backend pane is a source-aware audit log. The case-scoped handler chat is a compact, light handler-side panel opened from a calm green launcher over the left claim pane; its answers reflect the current preparation stage and preserve all human decision boundaries.
...
- Browser tests pass for the noise-free 16-row queue and hover-only demo guidance, Lina's first-person statement, responsive dormant/active pane layouts, active lifecycle checks, compact stage-aware agent chat, transparent AI-authored preparation copy, compact editable draft and explicit synthetic-send approval, automatic and manually controlled in-place message expansion, slower writing and incoming-email states, customer-source registration of the updated estimate, automatic first exchange, manual updated-estimate continuation, newest-first communication ordering, handler-ready transition, the separate Erik/Samsung evidence, exact prompt-injection location marker, fail-closed specialist referral, audit log, grouped source viewer, and reset.
