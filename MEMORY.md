...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. The compact draft remains editable and requires explicit handler approval; after that synthetic send, customer typing, the missing-photo reply, AI processing, and the AI acknowledgement play automatically. The flow then pauses for exploration until the tester triggers the updated-estimate email, after which the agent prepares the handler handoff. Newest messages appear first in a conversational layout with AI bubbles on the left and customer bubbles on the right; every message can open in a large readable dialog, newly arrived messages open automatically for about 2.3 seconds, incoming mail rests for about 2.6 seconds before processing, and matching left/right writing indicators remain for about 2.5 seconds. No real email is sent.
- The lower backend pane is a source-aware audit log. The case-scoped handler chat is a left-side handler tool opened from a persistent launcher into a drawer over the claim pane; its answers reflect the current preparation stage and preserve all human decision boundaries.
...
- Customer drafts, next actions, and ordinary route changes require explicit handler approval in the UI. The only exception is the deterministic security referral that stops Case 2 and sends it to Human Specialist Review without making a claim decision.
...
- Browser tests pass for the noise-free 16-row queue and hover-only demo guidance, Lina's first-person statement, responsive dormant/active pane layouts, active lifecycle checks, stage-aware agent chat, transparent AI-authored preparation copy, compact editable draft and explicit synthetic-send approval, automatic and manually reopenable large message views, paced writing and incoming-email states, customer-source registration of the updated estimate, automatic first exchange, manual updated-estimate continuation, newest-first communication ordering, handler-ready transition, the separate Erik/Samsung evidence, exact prompt-injection location marker, fail-closed specialist referral, audit log, grouped source viewer, and reset.
...
- Treat the generated customer email as a compact, editable step inside the communication log. It requires handler approval before the synthetic send; the demo never sends externally.
