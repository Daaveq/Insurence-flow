...
- Published stable branch: `main`; `fix/analysis-timeout` contains the recoverable stalled-agent fail-safe and has not been merged or deployed.
...
- The demo first leaves the claims overview unobstructed for two seconds, then opens a four-step darkened spotlight guide in a fixed narrative order: welcome, Case Handler portfolio, AI Agents/Codex workspace, then both interactive claims together. The final step explains both scenarios while asking the evaluator to begin with Lina. The first claim opened gets the ten-step workspace guide covering the Human Agent view, preparation map, live Codex window, AGENT.md, Rules.md, customer evidence, audit log, case chat, case reset, and live start control. That full guide does not repeat in the same demo session; Erik gets only a centered one-step case-specific introduction without a spotlight cutout on first entry. The source viewer follows the source-related full-guide steps automatically. Guide cards remain 440px wide on desktop with 13px body copy and responsive mobile sizing; guides can be skipped or dismissed with `Got it` where applicable.
- Generated repair-estimate reviews are rendered below the original white documents. Lina's updated estimate review explicitly confirms that it is the requested revision and that the missing IMEI is now present. In Case 2 a red marker appears at the bottom of the estimate showing the exact hidden-text location and wording, with a separate red security review underneath. Detection automatically scrolls the nested document preview to bring the red marker into view on the agent side.
...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. Case 1 pauses only after the first synthetic email has been sent, after the rear photo has been reviewed, and after the revised estimate has been reviewed. The first information dialog appears one second after the sent email. Routine writing states last five seconds; after the photo acknowledgement, Lina visibly writes for five seconds before the updated-estimate email appears. Incoming-photo and estimate-review moments otherwise continue automatically. Each checkpoint begins with a brief dark pulse and a centered information dialog that can be dismissed with `Got it`, while the communication log and timeline remain visibly active on the next writing, reading, or monitoring action. Relevant source highlights remain active and the evaluator can keep exploring. A discreet fixed `Continue demo` control separately advances playback. Lina's completion dialog appears one second after `Ready for handler review`, explains that the file was prepared before handler pickup, and asks the evaluator to open Erik manually; Erik's completion dialog appears five seconds after the stopped-review state, then suggests further exploration or an overview-level `Reset Demo`. Neither completion uses a navigation button or overview next-case prompt. Newest messages remain first and expandable. No real email is sent.
...
- Browser tests pass for the two-second unobstructed overview landing followed by the two-case Lina-first spotlight, 280px dormant agent pane, full workspace guide running only once, centered non-spotlit malicious-case introduction, three tester-controlled external checkpoints with dismissible dialogs and bounded chat status, one-second delays after both the first sent email and Lina's ready state, five-second writing states including the pre-estimate customer bubble, updated-estimate reasoning, continuous source highlights during exploration, discreet continuation controls, Lina and Erik completion dialogs, manual portfolio navigation, responsive desktop/mobile layouts, persistent overview outcomes, expandable newest-first communications, the separate Erik/Samsung evidence, exact prompt-injection location marker with automatic visible focus, fail-closed specialist referral, stalled-agent timeout recovery, audit log, grouped source viewer, and reset. The full static suite currently passes 7 tests with the authenticated live test skipped against an isolated production build.
...
- Treat generated customer emails as transparent synthetic playback inside the communication log. Drafting and intermediate customer exchanges remain automatic; the three selected review milestones advance only through tester-controlled continuation outside the product UI, and the demo never sends externally.
