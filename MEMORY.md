...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. Case 1 pauses only after the first synthetic email has been sent, after the rear photo has been reviewed, and after the revised estimate has been reviewed. Incoming-photo, waiting-for-estimate, and estimate-arrival moments continue automatically. Each checkpoint begins with a brief dark pulse and a centered information dialog that can be dismissed with `Got it`, while the communication log and timeline remain visibly active on the next writing, reading, or monitoring action. Relevant source highlights remain active and the evaluator can keep exploring. A discreet fixed `Continue demo` control separately advances playback. Lina's completion dialog asks the evaluator to open Erik manually; Erik has his own completion dialog suggesting further exploration or an overview-level `Reset Demo`. Neither completion uses a navigation button or overview next-case prompt. Newest messages remain first and expandable. No real email is sent.
