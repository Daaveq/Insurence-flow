...
- The demo opens with a four-step darkened spotlight guide that explains the handler portfolio, the dormant claim-specific agent side, and the two interactive claims. Opening either claim automatically starts an eight-step contextual guide that explains the 50/50 workspace, the preparation map, each of its four lifecycle boxes, the bounded source/audit workspace, and where to start preparation. Guide cards are 440px wide on desktop with 13px body copy and larger headings and controls. The box copy adapts to the routine or malicious-document lifecycle, and either guide can be skipped.
...
- Reset Demo aborts an active request, clears that claim's overview outcome, and restores results, approvals, trace, draft edits, and source selection to the initial state. Returning through All claims preserves completed overview outcomes; starting a rerun clears the previous outcome until the new cycle finishes.
...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. Once the first transparent request is drafted, the synthetic demo automatically logs it as sent and continues into customer writing, the missing-photo reply, visible rear-photo ingestion, and the AI acknowledgement without evaluator approval. Each sent or received email now rests for 4 seconds before the next activity begins; every visible writing, ingestion, processing, or reply-drafting bubble remains for 9 seconds. The flow then pauses for exploration until the tester triggers the updated-estimate email, after which the agent prepares the handler handoff. Newest messages appear first, the latest remains expanded, and older messages expand or collapse in place. No real email is sent.
...
- Browser tests pass for the two-stage overview and claim-box spotlight guides with enlarged readable cards, the noise-free 16-row queue without demo-case sublabels and with hover-only demo guidance, persistent softly green Lina-ready and softly red Erik-security-attention overview outcomes, Lina's first-person statement, responsive dormant/active pane layouts, active lifecycle checks, bounded live Codex chat payloads and conversation history, transparent AI-authored preparation copy, automatic synthetic first-email playback without evaluator input, permanent expansion of the latest message plus manually controlled older-message expansion, 4-second landed-email pauses and 9-second activity bubbles, visible customer-source registration and processing of the rear photo, customer-source registration of the updated estimate, automatic first exchange, manual updated-estimate continuation, newest-first communication ordering, handler-ready transition, the separate Erik/Samsung evidence, exact prompt-injection location marker with automatic visible focus, fail-closed specialist referral, audit log, grouped source viewer, and reset.
