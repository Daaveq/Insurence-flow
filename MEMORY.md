...
- Published branch: `main`; current task branch: `feat/nonblocking-demo-pauses`.
...
- The demo opens with a four-step darkened spotlight guide in a fixed narrative order: welcome, Case Handler portfolio, AI Agents/Codex workspace, then the two interactive claims. The final overview step presents both scenarios but explicitly asks the evaluator to start with Lina and return for Erik. Opening either claim starts a ten-step guide covering the Human Agent view, preparation map, live Codex window, AGENT.md, Rules.md, customer evidence, audit log, case chat, case reset, and live start control. The source viewer follows the source-related steps automatically: AGENT.md opens AGENT.md, Rules.md opens Rules.md, and Customer evidence opens Damage.jpg, with the viewer returned to the top. It no longer tours the four preparation boxes individually. Guide cards remain 440px wide on desktop with 13px body copy and responsive mobile sizing; either guide can be skipped.
...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. After the evidence review and transparent first email are prepared, playback pauses before the synthetic send while the communication log remains visibly active on a pulsing “Writing email” bubble. A brief dark spotlight pulse introduces floating guidance outside the product UI; the evaluator can continue exploring sources, timeline, audit log, and chat, then use a discreet fixed “Continue the demo” control. Continuation logs the first email as sent and runs customer writing, the missing-photo reply, rear-photo ingestion, acknowledgement, updated-estimate arrival, processing, and handler handoff without another manual trigger. Each sent or received email rests for 4 seconds before the next activity begins; every visible writing, ingestion, processing, or reply-drafting bubble remains for 9 seconds. Completion uses the same floating treatment and asks the evaluator to return through All claims and open Erik’s case manually; no overview-level next-case notification is shown. Newest messages appear first, the latest remains expanded, and older messages expand or collapse in place. No real email is sent.
...
- Browser tests pass for the ordered four-step overview and ten-step claim/Codex spotlight guides with enlarged readable cards, the two-case overview that directs evaluators to Lina first, the noise-free 16-row queue without demo-case sublabels and with hover-only demo guidance, persistent softly green Lina-ready and softly red Erik-security-attention overview outcomes, Lina's first-person statement, responsive dormant/active pane layouts, active lifecycle checks, bounded live Codex chat payloads and conversation history, transparent AI-authored preparation copy, the non-blocking evidence-review pause with brief dark pulse and persistent writing activity, source exploration during the pause, discreet external continuation, automatic post-continue customer exchange and estimate arrival, completion guidance with manual navigation and no overview next-case prompt, permanent expansion of the latest message plus manually controlled older-message expansion, 4-second landed-email pauses and 9-second activity bubbles, visible customer-source registration and processing of the rear photo, customer-source registration of the updated estimate, newest-first communication ordering, handler-ready transition, the separate Erik/Samsung evidence, exact prompt-injection location marker with automatic visible focus, fail-closed specialist referral, audit log, grouped source viewer, and reset.
