...
- Communications are the central routine-case stage and identify AI, customer, internal, and safety-control actors. Case 1 now pauses at six explicit comprehension checkpoints: before the first synthetic email, after the rear-photo email arrives, after the photo review, after the AI acknowledgement, after the revised estimate arrives, and after the estimate review before the final reply. Each checkpoint explains what just happened, makes clear what has not happened yet, and waits indefinitely for `Continue demo`; source working highlights stop while paused. Writing, ingestion, review, and reply animations remain visible between checkpoints. Completion offers `Back to both cases`, and the portfolio then presents a prominent next-step card for Erik's malicious-document case. Newest messages remain first and expandable. No real email is sent.
...
- Browser tests pass for the Lina-first overview spotlight, 280px dormant agent pane, full workspace guide running only once, one-step malicious-case introduction, six tester-controlled routine-case checkpoints with bounded chat status, active-only source highlighting, completion-to-portfolio handoff and Erik prompt, responsive desktop/mobile layouts, persistent overview outcomes, expandable newest-first communications, the separate Erik/Samsung evidence, exact prompt-injection location marker with automatic visible focus, fail-closed specialist referral, audit log, grouped source viewer, and reset. The full static suite currently passes 6 tests with the authenticated live test skipped.
...
- Treat generated customer emails as transparent synthetic playback inside the communication log. Drafting remains automatic, but every send, review, and receipt segment advances only through clearly labeled tester-controlled checkpoints; the demo never sends externally.
